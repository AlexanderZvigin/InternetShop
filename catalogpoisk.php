...
error_reporting(0);
var_dump($_POST);
$poisk=$_POST['Sortpoisk'];
echo $poisk;
$n=NULL;
include 'dbcatalog.php';
$db = connect_db();
$goods = get_goods($db);
$sql="SELECT * FROM `products` WHERE Nazvanie LIKE '%$poisk%' OR id LIKE '$poisk'";
$result = mysqli_query($db,$sql);
 for($i = 0;$i < mysqli_num_rows($result); $i++) {
  $goodspoisk[] = mysqli_fetch_array($result);
 }
   var_dump($goodspoisk);
?>
...
<div class="product-item">
	<?php foreach ($goodspoisk as $item) : ?>
